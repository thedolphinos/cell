...
class ApplicationService extends Service
{
  ...
  async partialUpdateOne (query, documentCandidate, options = {})
  {
    ...
    query = this._adaptQuery(query);
    documentCandidate = this._validateAndConvertDocumentCandidate(documentCandidate, this.dbOperation.schema.definition);
    documentCandidate = await this._hook_updateOne_documentCandidate(documentCandidate);

    if (utility.isExist(options.session))
    {
      let documents = await this.read(query, options);
      documents = await this._hook_updateOne_afterReadOperation(documents);
      ...
      documentCandidate = await this._hook_updateOne_beforeUpdateOperation(document, documentCandidate);
      ...
      result = result.value;
      result = await this._hook_updateOne_afterUpdateOperation(result);
   ...
   * @protected
   */
  async _hook_updateOne_documentCandidate (documentCandidate)
  {
    if (!_.isPlainObject(documentCandidate))
    {
      throw new InvalidArgumentsError();
   ...
   * @protected
   */
  async _hook_updateOne_afterReadOperation (documents)
  {
    if (!this._isCrudControllerApplicationService)
    {
      if (!_.isArray(documents))
   ...
   * @protected
   */
  async _hook_updateOne_beforeUpdateOperation (document, documentCandidate)
  {
   ...
   * @protected
   */
  async _hook_updateOne_afterUpdateOperation (document)
  {
    // since result may be null, it must be checked if exists.
    if (utility.isExist(document) && !_.isObject(document))
    {
