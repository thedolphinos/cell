...
class ApplicationService extends Service
{
  ...
  async readOneById (_id, options = {}, hooks = {})
  {
    ...
    return this.readOne({_id}, options, hooks);
  }
   ...
   *        before           : (documentCandidate, [session]) -> void
   *        after            : (document, [session]) -> void
  ...
  async updateOne (query, documentCandidate, options = {}, hooks = {})
  {
    ...
    if (utility.isExist(options.session))
    {
      ...
      document = await this._dbOperation.findOneAndUpdate({_id: document._id}, this._prepareUpdateOperation(documentCandidate), {
        ...options,
        session
      });
      utility.isExist(hooks.after) ? await hooks.after(document, session) : null;
   ...
   * @returns {Promise<Object>} - The updated document.
   */
  async updateOneByIdAndVersion (_id, version, documentCandidate, options = {}, hooks = {})
  {
    if (!_.isPlainObject(documentCandidate) ||
        !_.isPlainObject(options))
    {
    ...
    if (utility.isExist(options.session))
    {
      ...
      document = await this.updateOne({_id}, documentCandidate, options, hooks);
  ...
  /**
   * Replaces the matching document (if there is only 1) with the specified query, document candidate, and options.
   * Uses transactions. If a session is not provided externally, creates one internally.
   *
   * @param {Object} query
   * @param {Object} documentCandidate
   ...
   * This method is a specialized version of the method `replaceOne`. All hooks of it also is applied to this.
   *
   * @param {string | ObjectId} _id
   * @param {string | number} version
   * @param {Object} documentCandidate
   ...
   * @returns {Promise<Object>} - The updated document.
   */
  async replaceOneByIdAndVersion (_id, version, documentCandidate, options = {}, hooks = {})
  {
    if (!_.isPlainObject(documentCandidate) ||
        !_.isPlainObject(options))
    {
    ...
    if (utility.isExist(options.session))
    {
      ...
      document = await this.replaceOne({_id}, documentCandidate, options, hooks);
        ...
        await session.withTransaction(async () =>
                                      {
                                        ...
                                        document = await this.replaceOne({_id}, documentCandidate, {
                                          ...options,
                                          session
                                        }, hooks);
  ...
  async softDeleteOne (query, options = {}, hooks = {})
  {
    ...
    return this.updateOne(query, this._generateDocumentPropertyCandidatesForSoftDelete(), options, hooks);
  }
  ...
  async softDeleteOneByIdAndVersion (_id, version, options = {}, hooks = {})
  {
    ...
    return this.updateOneByIdAndVersion(_id, version, this._generateDocumentPropertyCandidatesForSoftDelete(), options, hooks);
  }
  ...
  async deleteOne (query, options = {}, hooks = {})
  {
    ...
    if (utility.isExist(options.session))
    {
      ...
      utility.isExist(hooks.before) ? await hooks.before(query, session) : null;
      document = await this._dbOperation.findOneAndDelete({_id: document._id}, {
        ...options,
        session
      });
      utility.isExist(hooks.after) ? await hooks.after(document, session) : null;
  ...
  async deleteOneByIdAndVersion (_id, version, options = {}, hooks = {})
  {
    ...
    if (utility.isExist(options.session))
    {
      ...
      document = await this.deleteOne({_id}, options, hooks);
