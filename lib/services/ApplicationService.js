...
class ApplicationService extends Service
{
  ...
  async read (query, options = {})
  {
    ...
    documents = await this._hook_read_afterReadOperation(documents);
  ...
  async readOne (query, options = {})
  {
    ...
    document = await this._hook_readOne_afterReadOperation(document);
  ...
  async createOne (documentCandidate, options = {})
  {
    ...
    document = await this._hook_createOne_afterCreateOperation(document);
   ...
   */
  async partialUpdateOne (query, documentCandidate, options = {})
  {
    if (!_.isPlainObject(query) ||
    ...
    query = this._adaptQuery(query);
    documentCandidate = this._validateAndConvertDocumentCandidate(documentCandidate, this.dbOperation.schema.definition);
    documentCandidate = await this._hook_partialUpdateOne_documentCandidate(documentCandidate);

    if (utility.isExist(options.session))
    {
      let documents = await this.read(query, options);
      documents = await this._hook_partialUpdateOne_afterReadOperation(documents);
      ...
      documentCandidate = await this._hook_partialUpdateOne_beforeUpdateOperation(document, documentCandidate);

      let result = await this._dbOperation.getNativeOps().findOneAndUpdate({_id: document._id}, this._prepareUpdateOperation(documentCandidate), {
        ...options,
        returnOriginal: false
      });
      result = result.value;
      result = await this._hook_partialUpdateOne_afterUpdateOperation(result);
  ...
  async partialUpdateOneByIdAndVersion (_id, version, documentCandidate, options = {})
  {
    ...
    if (utility.isExist(options.session))
    {
      const document = await this.partialUpdateOne({_id}, documentCandidate, options);
    ...
    try
    {
      await internalSession.withTransaction(async () =>
                                            {
                                              document = await this.partialUpdateOne({_id}, documentCandidate, {
                                                ...options,
                                                session: internalSession
                                              });
  ...
  async softDeleteOne (query, options = {})
  {
    ...
    return await this.partialUpdateOne(query, this._generateDocumentPropertyCandidatesForSoftDelete(), options);
  }
  ...
  async softDeleteOneByIdAndVersion (_id, version, options = {})
  {
    ...
    return await this.partialUpdateOneByIdAndVersion(_id, version, this._generateDocumentPropertyCandidatesForSoftDelete(), options);
  }
  ...
  async deleteOne (query, options = {})
  {
    ...
    {
      let documents = await this.read(query, options);
      documents = await this._hook_deleteOne_afterReadOperation(documents);
      const document = documents[0];
      ...
      result = result.value;
      result = await this._hook_deleteOne_afterDeleteOperation(result);
      return document;
    }
  ...
  _prepareUpdateOperation (documentCandidate)
  {
    ...
    const keys = Object.keys(convertedDocumentCandidate);
   ...
   * @protected
   */
  async _hook_read_afterReadOperation (documents)
  {
   ...
   * @protected
   */
  async _hook_readOne_afterReadOperation (document)
  {
  ...
  /**
   * Hooks to document candidate for the method `createOne`.
   *
   * @since 0.24.0
   * @param {Object} documentCandidate
   ...
   * @protected
   */
  async _hook_createOne_afterCreateOperation (document)
  {
  ...
  /**
   * Hooks to document candidate for the method `partialUpdateOne`.
   *
   * @since 0.24.0
   * @param {Object} documentCandidate
   * @returns {Promise<Object>}
   * @protected
   */
  async _hook_partialUpdateOne_documentCandidate (documentCandidate)
  {
    if (!_.isPlainObject(documentCandidate))
    {
      throw new InvalidArgumentsError();
  ...
  /**
   * Hooks to read result for the method `partialUpdateOne`.
   *
   * @since 0.22.0
   * @param {Array<Object>} documents
   * @protected
   */
  async _hook_partialUpdateOne_afterReadOperation (documents)
  {
    if (!this._isCrudControllerApplicationService)
    {
      if (!_.isArray(documents))
  ...
  /**
   * Hooks to the method `partialUpdateOne`. Runs before the update operation.
   *
   * @since 0.25.0
   * @param {Object} document
   * @param {Object} documentCandidate
   ...
   * @protected
   */
  async _hook_partialUpdateOne_beforeUpdateOperation (document, documentCandidate)
  {
  ...
  /**
   * Hooks to result for the method `partialUpdateOne`.
   *
   * @since 0.22.0
   * @param {Object} document
   * @protected
   */
  async _hook_partialUpdateOne_afterUpdateOperation (document)
  {
    // since result may be null, it must be checked if exists.
    if (utility.isExist(document) && !_.isObject(document))
    {
   ...
   * @protected
   */
  async _hook_deleteOne_afterReadOperation (documents)
  {
    if (!this._isCrudControllerApplicationService)
    {
      if (!_.isArray(documents))
   ...
   * @protected
   */
  async _hook_deleteOne_afterDeleteOperation (document)
  {
    // since result may be null, it must be checked if exists.
    if (utility.isExist(document) && !_.isObject(document))
    {
