...
class ApplicationService extends Service
{
  ...
  constructor (options)
  {
    ...
    this._rootDbService = this.schema.isVersionBased ? new DbService({schema: this.schema.rootSchema}) : undefined;
  }
  ...
  async createOne (documentCandidate, externalSession = undefined, hooks = undefined)
  {
    ...
    let document;
    this.schema.isVersionBased ? hooks.isSessionEnabled = true : undefined;
    const {session, internalSession} = SessionManager.generateSessionsForService(externalSession, hooks);
    await SessionManager.exec(async () =>
                              {
                                ...
                                if (this.schema.isVersionBased)
                                {
                                  const rootDocument = await this._rootDbService.createOne({version: 0, isSoftDeleted: false}, {session});
    ...
    await SessionManager.exec(async () =>
                              {
                                ...
                                if (this.schema.isVersionBased)
                                {
                                  ...
                                  await this._dbService.createOne(documentCandidate, {session});

                                  if (documentCandidate.isSoftDeleted)
                                  {
                                    await this._rootDbService.dbOperation.getNativeOps().updateOne({_id: document._root}, {$set: {isSoftDeleted: true}}, {session});
  ...
  _adaptQuery (query)
  {
    ...
    if (utility.isExist(query.$query))
    {
      ...
      query.$query.isSoftDeleted = false;
      this.schema.isVersionBased ? query.$query.isRecent = true : undefined;
    }
    else
    {
      query.isSoftDeleted = false;
      this.schema.isVersionBased ? query.isRecent = true : undefined;
    }
