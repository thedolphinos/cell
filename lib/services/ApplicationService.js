...
class ApplicationService extends Service
{
   ...
   * @param {Object} [options.dependencies] - Key value pair of names and injected application services using the injector.
   * @param {Array} [options.fromConstructorParameters] - Must be presented if dependencies are presented.
   * @param {ApplicationService} [options.from] - Placed by the framework.
   */
  constructor (options)
  {
    ...
    super(
      {
        ...
        fromConstructorParameters: options.fromConstructorParameters,
        from: options.from
      }
    );
    ...
    if (utility.isExist(options.schema))
    {
      ...
      this._dbService = new DbService(options.schema);
    ...
    if (utility.isExist(options.dbOperation))
    {
      ...
      this._dbService = new DbService(options.dbOperation);
      isDbServiceCreated = true;
    }
  ...
  _checkVersion (documentVersion, version)
  {
    ...
    {
      throw new BadRequestError(ERROR_DATA.DOCUMENT_INVALID_VERSION, documentVersion, version);
    }
    else if (documentVersion > version)
    {
      throw new BadRequestError(ERROR_DATA.DOCUMENT_MODIFIED, documentVersion, version);
    }
  }
