...
class CrudControllerService extends Service
{
  ...
  async updateOneByIdAndVersion (_id, version, documentCandidate, options = {})
  {
    ...
    await this._hook_updateOneByIdAndVersion_id_version(_id, version);
    documentCandidate = await this._hook_updateOneByIdAndVersion_documentCandidate(documentCandidate);
    options = await this._hook_updateOneByIdAndVersion_options(options);
   ...
   * @protected
   */
  async _hook_updateOneByIdAndVersion_id_version (_id, version)
  {
    if (!utility.isObjectId(_id) ||
   ...
   * @protected
   */
  async _hook_updateOneByIdAndVersion_documentCandidate (documentCandidate)
  {
    if (!_.isPlainObject(documentCandidate))
    {
      throw new InvalidArgumentsError();
   ...
   * @protected
   */
  async _hook_updateOneByIdAndVersion_options (options)
  {
    if (!_.isPlainObject(options))
    {
      throw new InvalidArgumentsError();
