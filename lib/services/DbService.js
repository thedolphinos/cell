...
class DbService extends Service
{
  ...
  async read (query, options = {}, hooks = {})
  {
    ...
    await this._exec(async () =>
                     {
                       utility.isExist(hooks.query) ? query = await hooks.query(query, options.session) : query;

                       utility.isExist(hooks.before) ? await hooks.before(query, options.session) : null;
                       documents = await this._dbOperation.read(query, options);
                       utility.isExist(hooks.after) ? await hooks.after(documents, options.session) : null;
                     }, options.session, hooks.isSessionEnabled);
  ...
  async readOne (query, options = {}, hooks = {}, hooksOfSpecializedVersion = {})
  {
    ...
    await this._exec(async () =>
                     {
                       utility.isExist(activeHooks.query) ? query = await activeHooks.query(query, options.session) : query;

                       utility.isExist(activeHooks.before) ? await activeHooks.before(query, options.session) : null;
                       document = await this._dbOperation.readOne(query, options);
                       utility.isExist(activeHooks.after) ? await activeHooks.after(document, options.session) : null;
                     });
  ...
  async createOne (documentCandidate, options = {}, hooks = {})
  {
    ...
    await this._exec(async () =>
                     {
                       utility.isExist(hooks.documentCandidate) ? await hooks.documentCandidate(documentCandidate, options.session) : null;

                       utility.isExist(hooks.before) ? await hooks.before(documentCandidate, options.session) : null;
    ...
    await this._exec(async () =>
                     {
                       ...
                       utility.isExist(hooks.before) ? await hooks.before(query, document, documentCandidate, options.session) : null;
                       document = await this._dbOperation.findOneAndUpdate({_id: document._id}, documentCandidate, {
                         ...options,
                         returnOriginal: false
                       });
                       utility.isExist(hooks.after) ? await hooks.after(document, options.session) : null;
                     }, options.session, hooks.isSessionEnabled);
