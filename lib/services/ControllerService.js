...
const {DbError, InvalidArgumentsError, BadRequestError} = require("@thedolphinos/error4js");
...
class ControllerService extends Service
{
   ...
   * @param {Object} [parentRoutes]
   * @param {Object} [hooks]
   * @returns {Promise<Array>} - The fetched documents.
   */
  async read (query, externalSession = null, parentRoutes = {}, hooks = {})
  {
    ...
    await SessionManager.exec(async () =>
                              {
                                ...
                                if (utility.isInitialized(parentRoutes))
                                {
                                  ...
                                  for (let i = 0; i < parentRoutes.length; i++)
                                  {
                                    ...
                                    query[parentRoute.pathParameter] = parentRoute.value;

                                    if (i === parentRoutes.length - 1)
                                    {
                                      const document = await parentRoute.applicationService.readOne(query, session);

                                      if (!utility.isExist(document))
                                      {
                                        throw new BadRequestError("No such relation between routes.");
                                      }
                                    }
   ...
   */
  async readOneById (_id, externalSession = null, parentRoutes = {}, hooks = {})
  {
    this._validateParameterSession(externalSession);
    this._validateParameterHooks(hooks);
                                ...
                                if (utility.isInitialized(parentRoutes))
                                {
                                  ...
                                  for (let i = 0; i < parentRoutes.length; i++)
                                  {
                                    ...
                                    if (i === parentRoutes.length - 1)
                                    {
                                      ...
                                      try
                                      {
                                        document = await this.applicationService.readOne(query, session);
                                      }
                                      catch (error)
                                      {
                                        // ignore DB level errors.
   ...
   */
  async createOne (body, externalSession = null, parentRoutes = {}, hooks = {})
  {
   ...
   */
  async updateOneByIdAndVersion (_id, version, documentCandidate, externalSession = null, parentRoutes = {}, hooks = {})
  {
    ...
    await SessionManager.exec(async () =>
                              {
                                ...
                                if (utility.isInitialized(parentRoutes))
                                {
                                  ...
                                  for (let i = 0; i < parentRoutes.length; i++)
                                  {
                                    ...
                                    if (i === parentRoutes.length - 1)
                                    {
                                      const document = await this.applicationService.readOne(query, session);
   ...
   * @return {Promise<Object>}
   */
  async replaceOneByIdAndVersion (_id, version, documentCandidate, externalSession = null, parentRoutes = {}, hooks = {})
  {
    this._validateParameterDocumentCandidate(documentCandidate);
    this._validateParameterSession(externalSession);
    this._validateParameterHooks(hooks);
   ...
   */
  async softDeleteOneByIdAndVersion (_id, version, externalSession = null, parentRoutes = {}, hooks = {})
  {
    this._validateParameterSession(externalSession);
    this._validateParameterHooks(hooks);
   ...
   */
  async deleteOneByIdAndVersion (_id, version, externalSession = null, parentRoutes = {}, hooks = {})
  {
    this._validateParameterSession(externalSession);
    this._validateParameterHooks(hooks);
