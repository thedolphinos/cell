...
class ControllerService extends Service
{
  ...
  /**
   * Fetches the documents that match the specified search value on each specified fields.
   *
   ...
   */
  async search (searchValue, searchFields, externalSession = undefined, hooks = undefined)
  {
    ...
    await SessionManager.exec(async () =>
                              {
                                ...
                                documents = await this.dbOperation.read(convertedQuery, {session});
