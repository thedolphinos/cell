...
class ControllerService extends Service
{
    ...
    await SessionManager.exec(async () =>
                              {
                                ...
                                const result = await this._applicationService.updateOneByIdAndVersion(_id, version, fields, session, {bearer: hooks.bearer});
                                document = result.document;
                                utility.isExist(hooks.after) ? await hooks.after(document, result.oldDocument, session) : undefined;
                              }, externalSession, internalSession);
  ...
  async replaceOneByIdAndVersion (_id, version, fields, externalSession = undefined, hooks = undefined)
  {
    ...
    await SessionManager.exec(async () =>
                              {
                                utility.isExist(hooks.before) ? await hooks.before(_id, version, fields, session) : undefined;
                                const result = await this._applicationService.replaceOneByIdAndVersion(_id, version, fields, session, {bearer: hooks.bearer});
                                document = result.document;
                                utility.isExist(hooks.after) ? await hooks.after(document, result.oldDocument, session) : undefined;
                              }, externalSession, internalSession);
    ...
    await SessionManager.exec(async () =>
                              {
                                ...
                                const result = await this._applicationService.softDeleteOneByIdAndVersion(_id, version, session, {bearer: hooks.bearer});
                                document = result.document;
                                utility.isExist(hooks.after) ? await hooks.after(document, result.oldDocument, session) : undefined;
                              }, externalSession, internalSession);
  ...
  async deleteOneByIdAndVersion (_id, version, externalSession = undefined, hooks = undefined)
  {
    ...
    await SessionManager.exec(async () =>
                              {
                                utility.isExist(hooks.before) ? await hooks.before(_id, version, session) : undefined;
                                const result = await this._applicationService.deleteOneByIdAndVersion(_id, version, session, {bearer: hooks.bearer});
                                document = result.document;
                                utility.isExist(hooks.after) ? await hooks.after(document, result.oldDocument, session) : undefined;
                              }, externalSession, internalSession);
   ...
   * Uses transactions separately for each document. If a session is not provided externally, creates one internally, if the hooks is not disabling session.
   *
    ...
    for (const {_id, version} of documents)
    {
      try
      {
        ...
        await SessionManager.exec(async () =>
                                  {
                                    utility.isExist(hooks.before) ? await hooks.before(_id, version, session) : undefined;
                                    const {document, oldDocument} = await this._applicationService.softDeleteOneByIdAndVersion(_id, version, session, {bearer: hooks.bearer});
                                    utility.isExist(hooks.after) ? await hooks.after(document, oldDocument, session) : undefined;
