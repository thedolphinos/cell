...
class ControllerService extends Service
{
  ...
  async read (query, externalSession = null, hooks = {})
  {
    ...
    this._validateParamSession(externalSession);
    let internalSession;
   ...
   * Uses transactions. If a session is not provided externally, creates one internally, if hooks are not disabling sessions.
   *
   * @param {string | ObjectId} _id
   * @param {string | number} version
   * @param {Object} [externalSession]
