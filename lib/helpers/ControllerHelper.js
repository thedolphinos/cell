...
const {InvalidArgumentsError, StaticClassInstantiationError, HTTPError, HeadersMissingError, PathParametersMissingError, PathParameterMissingError, QueryStringMissingError, QueryStringParameterMissingError, BodyMissingError, InternalServerError} = require("@thedolphinos/error4js");
...
class ControllerHelper
{
  ...
  {
    return _.isPlainObject(request);
  }
   ...
   * @param {Function} next
   */
  static validateControllerParams (request, response, next = null)
  {
    if (!ControllerHelper.isValidRequest(request) ||
        !ControllerHelper.isValidResponse(response) ||
        !ControllerHelper.isValidNext(next))
    {
  ...
  static extractPathParameter (request, name)
  {
    ...
    const pathParameters = ControllerHelper.extractPathParameters(request);
  ...
  static extractQueryStringParameter (request, name)
  {
    ...
    const queryString = ControllerHelper.extractQueryString(request);
  ...
  {
    if (!ControllerHelper.isValidResponse(response) ||
        !utility.isValidNumber(statusCode) || !(HTTP_STATUS_CODES.INFORMATIONAL.includes(statusCode) || HTTP_STATUS_CODES.SUCCESSFUL.includes(statusCode)) ||
  ...
  {
    if (!ControllerHelper.isValidResponse(response) ||
        !(error instanceof HTTPError) || !(HTTP_STATUS_CODES.CLIENT_ERROR.includes(error.statusCode) || HTTP_STATUS_CODES.SERVER_ERROR.includes(error.statusCode)))
    {
