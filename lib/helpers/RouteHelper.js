...
const ROUTE_NAME = Object.freeze({
                                   ...
                                   PARTIAL_UPDATE_ONE_BY_ID_AND_VERSION: "PARTIAL_UPDATE_ONE_BY_ID_AND_VERSION",
...
const DEFAULT_ROUTE_PATHS = Object.freeze({
                                            ...
                                            READ_ONE_BY_ID: "/:id",
                                            CREATE_ONE: "",
                                            PARTIAL_UPDATE_ONE_BY_ID_AND_VERSION: "/:id",
                                            SOFT_DELETE_ONE_BY_ID_AND_VERSION: "/:id",
                                            DELETE_ONE_BY_ID_AND_VERSION: "/:id"
                                          });
...
class RouteHelper
{
  ...
  /**
   * Fetches default route paths that each corresponds to a route name.
   *
   * @since 0.33.0
   * @return {Readonly<{Object}>}
   */
  static getDefaultRoutePaths ()
  {
    return DEFAULT_ROUTE_PATHS;
  }
   ...
   * @param {string} [routes.PARTIAL_UPDATE_ONE_BY_ID_AND_VERSION]
  ...
  static generateRoutes (router, authController, controller, routes)
  {
    ...
    for (let i = 0; i < routeNames.length; i++)
    {
      ...
      if (!utility.isExist(ROUTE_NAME[routeName]) || !utility.isExist(routePath))
      {
        throw new InvalidArgumentsError();
      }
    }
    ...
    if (routeNames.includes(ROUTE_NAME.PARTIAL_UPDATE_ONE_BY_ID_AND_VERSION))
    {
      if (utility.isExist(authController))
      {
        router.route(routes.PARTIAL_UPDATE_ONE_BY_ID_AND_VERSION).patch((request, response, next) => authController.verify(request, response, next));
      }

      router.route(routes.PARTIAL_UPDATE_ONE_BY_ID_AND_VERSION).patch((request, response) => controller.partialUpdateOneByIdAndVersion(request, response));
    }

    if (routeNames.includes(ROUTE_NAME.SOFT_DELETE_ONE_BY_ID_AND_VERSION))
    {
      if (utility.isExist(authController))
      {
    ...
    if (routeNames.includes(ROUTE_NAME.DELETE_ONE_BY_ID_AND_VERSION))
    {
      if (utility.isExist(authController))
      {
