...
const ROUTE_NAME = Object.freeze({
                                   ...
                                   SOFT_DELETE_ONE_BY_ID_AND_VERSION_AND_VERSION: "SOFT_DELETE_ONE_BY_ID_AND_VERSION_AND_VERSION",
...
class RouteHelper
{
  ...
  static generateRoutes (router, authController, controller, routeNames)
  {
    ...
    if (routeNames.includes(ROUTE_NAME.READ_ONE_BY_ID))
    {
      ...
      router.route("/:id").get((request, response) => controller.readOneById(request, response));
    }
    ...
    if (routeNames.includes(ROUTE_NAME.CREATE_ONE))
    {
      ...
      router.route("/").post((request, response) => controller.createOne(request, response));
    }

    if (routeNames.includes(ROUTE_NAME.UPDATE_ONE_BY_ID_AND_VERSION))
    {
      if (utility.isExist(authController))
      {
      ...
      router.route("/:id").patch((request, response) => controller.updateOneByIdAndVersion(request, response));
    }

    if (routeNames.includes(ROUTE_NAME.SOFT_DELETE_ONE_BY_ID_AND_VERSION_AND_VERSION))
    {
      if (utility.isExist(authController))
      {
        router.route("/:id/soft").delete((request, response, next) => authController.verify(request, response, next));
      }

      router.route("/:id/soft").delete((request, response) => controller.softDeleteOneByIdAndVersion(request, response));
    }
    ...
    if (routeNames.includes(ROUTE_NAME.DELETE_ONE_BY_ID_AND_VERSION))
    {
      ...
      router.route("/:id").delete((request, response) => controller.deleteOneByIdAndVersion(request, response));
    }
  }
}
