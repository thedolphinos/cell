...
const ROUTE_NAME = Object.freeze({
                                   ...
                                   DELETE_ONE_SOFT_BY_ID: "DELETE_ONE_SOFT_BY_ID"
                                 });
...
class RouteHelper
{
  ...
  static generateRoutes (router, authController, controller, routeNames)
  {
    ...
    if (routeNames.includes(ROUTE_NAME.UPDATE_ONE_BY_ID))
    {
      ...
      router.route("/:id").patch((request, response) => controller.updateOneById(request, response));
    }
    ...
    if (routeNames.includes(ROUTE_NAME.DELETE_ONE_BY_ID))
    {
      ...
      router.route("/:id").delete((request, response) => controller.deleteOneById(request, response));
    }

    if (routeNames.includes(ROUTE_NAME.DELETE_ONE_SOFT_BY_ID))
    {
      if (utility.isExist(authController))
      {
      ...
      router.route("/:id/soft").delete((request, response) => controller.deleteOneSoftById(request, response));
    }
  }
}
