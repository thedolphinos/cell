...
class AuthController extends Controller
{
  ...
  constructor (applicationService, options)
  {
    ...
    this._validateConstructorParams(applicationService, options);

    this._applicationService = applicationService;
    this._applicationService.muteDbErrors = true;
   ...
   * @return {Promise<void>}
   */
  async register (request, response, next = null, hooks = {}, allowedProperties = {body: []})
  {
    try
    {
      ...
      utility.isExist(hooks.body) ? await hooks.body(body) : null;
      ...
      await SessionManager.exec(async () =>
                                {
                                  ...
                                  utility.isExist(hooks.payload) ? await hooks.payload(payload, account, session) : null;
   ...
   * @return {Promise<void>}
   */
  async login (request, response, next = null, hooks = {}, allowedProperties = {body: []})
  {
    try
    {
      this._validateControllerParams(request, response, next);
  ...
  async verify (request, response, next, hooks = {})
  {
    try
    {
      ...
      const payload = {_id: account._id};
      utility.isExist(hooks.payload) ? await hooks.payload(payload, account) : null;
