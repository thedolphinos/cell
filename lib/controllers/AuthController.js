...
class AuthController extends Controller
{
  ...
  /**
   * Cipher created using default encryption algorithm, private key, and init vector.
   *
   * @type {Cipher}
   * @private
   */
  ...
  /**
   * Decipher created using default encryption algorithm, private key, and init vector.
   *
   * @type {Decipher}
   * @private
   */
  ...
  constructor (applicationService, options = undefined)
  {
    ...
    this._cipher = crypto.createCipheriv(AuthController._DEFAULT_ENCRYPTION_ALGORITHM, this._privateKeyForEncryption, this._initVectorForEncryption);
    this._decipher = crypto.createDecipheriv(AuthController._DEFAULT_ENCRYPTION_ALGORITHM, this._privateKeyForEncryption, this._initVectorForEncryption);
  ...
  _generateToken (payload)
  {
    ...
    let encryptedToken = this._cipher.update(token, "utf-8", "hex");
    encryptedToken += this._cipher.final("hex");
  ...
  _decodeToken (encodedToken)
  {
    ...
    let decryptedToken = this._decipher.update(encodedToken, "hex", "utf-8");
    decryptedToken += this._decipher.final("utf8");
