...
class AuthController extends Controller
{
   ...
   * @param {string} options.privateKey - Private key to be used to sign token.
   * @param {number} [options.tokenLifetime] - The life time of a token in seconds.
  ...
  async verifyPublic (request, response, next, hooks = undefined, allowedPropertiesForHeaders = undefined)
  {
    try
    {
      if (!Validator.isValidParameterRequest(request) ||
          ...
          (!utility.isExist(allowedPropertiesForHeaders) || !Validator.isValidParameterAllowedPropertiesForRequestElements(allowedPropertiesForHeaders)))
      {
        throw new InvalidArgumentsError(ErrorSafe.get().DEV_1);
      }
  ...
  async verifyPrivate (request, response, next, hooks = undefined, allowedPropertiesForHeaders = undefined)
  {
    try
    {
      ...
      response.locals.oldToken = decodedToken.data;
   ...
   * @return {Promise<void>}
   */
  async register (request, response, next = undefined, hooks = undefined, propertyDefinition = undefined)
  {
    try
    {
      if (!Validator.isValidRouteFacingControllerMethodParameters(request, response, next, hooks, {body: propertyDefinition}))
      {
        throw new InvalidArgumentsError(ErrorSafe.get().DEV_1);
      }
      ...
      const body = AuthController._extractAndAuthorizeBody(request, propertyDefinition, true);
      ...
      await this._sendResponse(request, response, 200, {account});
    }
    catch (error)
    {
      this._sendResponseWhenError(response, error);
   ...
   * @return {Promise<void>}
   */
  async login (request, response, next = undefined, hooks = undefined, propertyDefinition = undefined)
  {
    try
    {
      if (!Validator.isValidRouteFacingControllerMethodParameters(request, response, next, hooks, {body: propertyDefinition}))
   ...
   * @return {Promise<void>}
   */
  async changePassword (request, response, next = undefined, hooks = undefined, propertyDefinition = undefined)
  {
    try
    {
      if (!Validator.isValidRouteFacingControllerMethodParameters(request, response, next, hooks, {body: propertyDefinition}))
  ...
  _decodeToken (encodedToken)
  {
    ...
    return jwt.verify(encodedToken, this._privateKey);
  }
}
