...
class AuthController extends Controller
{
  ...
  constructor (applicationService, options = undefined)
  {
    ...
    if (!_.isString(propertyNameOfUniqueIdentifier) || !utility.isInitialized(propertyNameOfUniqueIdentifier) ||
        ...
        (utility.isExist(isActivationEnabled) && (
  ...
  async register (request, response, next = undefined, hooks = undefined, allowedPropertiesForRequestElements = undefined)
  {
    try
    {
      ...
      await SessionManager.exec(async () =>
                                {
                                  ...
                                  const key = await crypto.generateKeySync("aes", {length: 256}); // for server-side encryption
                                  const passphrase = crypto.randomBytes(128).toString("hex");
                                  ...
                                  if (this._isActivationEnabled)
                                  {
                                    ...
                                    body.auth.activationCode = activationCode;

                                    utility.isExist(hooks.activation) ? await hooks.activation(activationCode, activationLink) : undefined;
                                  }
      ...
      // allows direct login after registration.
      if (!this._isActivationEnabled)
      {
        data.key = account.auth.encryption.publicKey;
        response.locals.authorizationBundle = await this._encodeAndEncryptAuthorizationBundle(account, {}, this._tokenPrivateKey, this._tokenLifetime);
      }
   ...
   * Checks if activation link is valid.
   * Cross-checks the activation codes.
  ...
  async activate (request, response, next = undefined, hooks = undefined, allowedPropertiesForRequestElements = undefined)
  {
    try
    {
      ...
      catch (error)
      {
        ...
        {
          throw new UnauthorizedError(ErrorSafe.get().ACTIVATION_LINK_MALFORMED);
        }
        else if (error.name === "TokenExpiredError")
        {
          throw new UnauthorizedError(ErrorSafe.get().ACTIVATION_LINK_EXPIRED);
        }
      ...
      {
        throw new UnauthorizedError(ErrorSafe.get().HTTP_22); // impossible.
      }

      if (activationLink !== account.auth.activationCode)
      {
        throw new UnauthorizedError(ErrorSafe.get().ACTIVATION_CODE_INVALID);
      }
      ...
      await SessionManager.exec(async () =>
                                {
                                  utility.isExist(hooks.before) ? await hooks.before(body, session) : undefined;
                                  await this.applicationService.updateOneByIdAndVersion(account._id, account.version,
                                                                                        {
                                                                                          auth: {
                                                                                            isActive: true,
                                                                                            activationCode: null
      ...
      await SessionManager.exec(async () =>
                                {
                                  ...
                                  if (!utility.isExist(error) &&
                                      !utility.isExist(response.locals.authorizationBundle))
                                  {
                                    response.locals.authorizationBundle = await this._encodeAndEncryptAuthorizationBundle(account, {}, this._tokenPrivateKey, this._tokenLifetime);
                                  }
                                },
   ...
   * Encrypts bundle using a server-specific key.
   *
  ...
  async _encodeAndEncryptAuthorizationBundle (account, tokenPayload, tokenPrivateKey, tokenLifeTime)
  {
    ...
    const bundle = {
      token: this._encrypt(token, account.auth.encryption.key),
      _account: account._id
    };

    return this._encrypt(JSON.stringify(bundle), this._encryptionKey);
  }
   ...
   *
   * @param {string} encryptedBundle - Encrypted bundle.
   * @param {string} tokenPrivateKey - Private key to be used to encode token.
   * @param {ApplicationService} applicationService - Application service to access the account.
   * @returns {Promise<Object>} - The account.
   * @private
   */
  async _decryptAndDecodeAuthorizationBundle (encryptedBundle, tokenPrivateKey, applicationService)
  {
    if (!_.isString(encryptedBundle) ||
        !_.isString(tokenPrivateKey) || !utility.isInitialized(tokenPrivateKey) ||
    ...
    const bundle = JSON.parse(this._decrypt(encryptedBundle, this._encryptionKey));

    const account = await applicationService.readOneById(bundle._account, null, {raiseDocumentExistenceErrors: true});

    const token = this._decrypt(bundle.token, account.auth.encryption.key);
  ...
  _encrypt (plainText, key)
  {
    ...
    const cipher = crypto.createCipheriv(AuthController.DEFAULT_ENCRYPTION_ALGORITHM, key, null);
    let encryptedMessage = cipher.update(plainText, encoding.input, encoding.output);
    encryptedMessage += cipher.final(encoding.output);
  ...
  _decrypt (encryptedText, key)
  {
    ...
    const decipher = crypto.createDecipheriv(AuthController.DEFAULT_ENCRYPTION_ALGORITHM, key, null);
