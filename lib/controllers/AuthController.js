...
class AuthController extends Controller
{
  ...
  async register (request, response, next = null, hooks = {})
  {
    try
    {
      ...
      await SessionManager.exec(async () =>
                                {
                                  ...
                                  utility.isExist(hooks.before) ? await hooks.before(accountCandidate, session) : null;
                                  ...
                                  const payload = utility.isExist(hooks.payload) ? await hooks.payload(account, session) : account._id;
      ...
      await this._sendResponse(response, 200, data);
    }
    catch (error)
    {
  ...
  async verify (request, response, next, hooks = {})
  {
    try
    {
      ...
      const account = await this._applicationService.readOneById(decodedToken._id);
      ...
      request.locals.newToken = this._generateToken(account);
