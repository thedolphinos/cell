  ...
  async activate (request, response, next = undefined, hooks = undefined, allowedPropertiesForRequestElements = undefined)
  {
    try
    {
      ...
      await SessionManager.exec(async () =>
                                {
                                  utility.isExist(hooks.before) ? await hooks.before(body, session) : undefined;
                                  account = await this.applicationService.updateOneByIdAndVersion(account._id, account.version,
                                                                                                  {
                                                                                                    auth: {
                                                                                                      isActive: true,
                                                                                                      activationCode: null
      ...
      await SessionManager.exec(async () =>
                                {
                                  ...
                                  utility.isExist(hooks.before) ? await hooks.before(account, newAccountProperties, session) : undefined;
                                  account = await this._applicationService.updateOneByIdAndVersion(account._id, account.version, newAccountProperties, session, {bearer: hooks.bearer});
                                  utility.isExist(hooks.after) ? await hooks.after(account, session) : undefined;
