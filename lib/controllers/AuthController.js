...
class AuthController extends Controller
{
      ...
      await SessionManager.exec(async () =>
                                {
                                  ...
                                  {
                                    throw new ForbiddenError(ERROR_DATA.AUTH_ACCOUNT_EXIST);
                                  }
  ...
  async login (request, response, next = undefined, hooks = undefined, allowedProperties = undefined)
  {
    try
    {
      ...
      if (!_.isString(uniqueIdentifier) ||
          !_.isString(password))
      {
        throw new BadRequestError(ERROR_DATA.AUTH_LOGIN_CREDENTIALS_MISSING);
      }
  ...
  async changePassword (request, response, next = undefined, hooks = undefined, allowedProperties = undefined)
  {
    try
    {
      ...
      const pathParameters = this._extractPathParameters(request, ["_id"]);
      ...
      await SessionManager.exec(async () =>
                                {
                                  ...
                                  if (!isPasswordCorrect)
                                  {
                                    error = new InvalidCredentialsError(); // not thrown to increase the number of failed change password attempts or block account.
                                  ...
                                  newAccountProperties[this._propertyNameOfPassword] = await bcrypt.hash(newPassword, await bcrypt.genSalt(10));
  ...
  async verifyPrivate (request, response, next, hooks = undefined, allowedProperties = undefined)
  {
    try
    {
      ...
      {
        throw new UnauthorizedError(ERROR_DATA.AUTH_DATA_MISSING);
      }
      ...
      if (account.auth.isBlocked)
      {
        throw new UnauthorizedError(ERROR_DATA.AUTH_ACCOUNT_BLOCKED);
      }
