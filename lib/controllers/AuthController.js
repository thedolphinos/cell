...
/**
 * A controller which is responsible from authorization and authentication.
...
class AuthController extends Controller
{
   ...
   * @param {Function} next
   * @param {Object} [hooks]
   * @param {{[headers]: Array<string>}} [allowedPropertyMap]
   * @return {Promise<void>}
   */
  async verifyPublic (request, response, next, hooks = undefined, allowedPropertyMap = undefined)
  {
    try
    {
      hooks = utility.init(hooks, {});
      hooks.bearer = utility.init(hooks.bearer, {});
      allowedPropertyMap = utility.init(allowedPropertyMap, {});
      allowedPropertyMap.headers = utility.init(allowedPropertyMap.headers, ["*"]);

      const headers = AuthController._extractAndAuthorizeHeaders(request, allowedPropertyMap.headers);
  ...
  async register (request, response, next = undefined, hooks = undefined, allowedProperties = undefined)
  {
    try
    {
      ...
      let body = AuthController._extractAndAuthorizeBody(request, allowedProperties.body);
