...
class CrudController extends Controller
{
  ...
  constructor (controllerService)
  {
    ...
    this._validateConstructorParameters(controllerService);

    this._controllerService = controllerService;
  }

  /**
   * Gets the related controller service.
   *
  ...
  get dbOperation ()
  {
    return this._controllerService.applicationService.dbOperation;
  }
  ...
  async readOneById (request, response, next = null, hooks = {})
  {
    ...
    try
    {
      ...
      const id = this._extractPathParameter(request, "id");
  ...
  async replaceOneByIdAndVersion (request, response, next = null, hooks = {})
  {
    ...
    try
    {
        ...
        await session.withTransaction(async () =>
                                      {
                                        ...
                                        utility.isExist(hooks.before) ? await hooks.before(id, version, body, session) : null;
                                        document = await this._controllerService.replaceOneByIdAndVersion(id, version, body, session);
                                        utility.isExist(hooks.after) ? await hooks.after(document, session) : null;
                                      }, this._controllerService.applicationService.dbOperation.transactionOptions);
      }
      else
      {
        ...
        utility.isExist(hooks.before) ? await hooks.before(id, version, body) : null;
        document = await this._controllerService.replaceOneByIdAndVersion(id, version, body);
        utility.isExist(hooks.after) ? await hooks.after(document) : null;
      }
