...
class CrudController extends Controller
{
  ...
  async createOne (request, response, next = null, hooks = {})
  {
    ...
    try
    {
      ...
      utility.isExist(hooks.session) ? session = await hooks.session() : null;
  ...
  async partialUpdateOneByIdAndVersion (request, response, next = null, hooks = {})
  {
    ...
    try
    {
        ...
        await session.withTransaction(async () =>
                                      {
                                        ...
                                        document = await this._controllerService.partialUpdateOneByIdAndVersion(id, version, body, {session});
                                        utility.isExist(hooks.after) ? await hooks.after(document, {session}) : null;
                                      }, this._controllerService.applicationService.dbOperation.transactionOptions);
      }
      else
      {
        ...
        document = await this._controllerService.partialUpdateOneByIdAndVersion(id, version, body);
        utility.isExist(hooks.after) ? await hooks.after(document) : null;
      }
   ...
   *
   * body
   *   createOne                     : (body, [options]) -> body
   *   partialUpdateOneByIdAndVersion: "
   *
   * before
   *   createOne                     : (body, [options]) -> void
   *   partialUpdateOneByIdAndVersion: (id, version, body, [options]) -> void
   *
   * after
   *   createOne                     : (document, [options]) -> void
   *   partialUpdateOneByIdAndVersion: "
   */
}
