...
class CrudController extends Controller
{
   ...
   * @param {Array<{pathParameter: string, localProperty: string, applicationService: ApplicationService}>} [options.parentRoutes] - The order of the parent routes are important. The preceding path comes first in the URL.
   */
  constructor (controllerService, options = {})
  {
    super();

    options.allowedProperties = utility.isExist(options?.allowedProperties) ? options.allowedProperties : CrudController.DEFAULT_ALLOWED_PROPERTIES;
    options.isAllowedPropertiesComplete = utility.isExist(options?.isAllowedPropertiesComplete) ? options.isAllowedPropertiesComplete : false;

    const parentRoutes = options?.parentRoutes;
    if (utility.isExist(parentRoutes))
    {
      Validator.validateParentRoutes(parentRoutes);
    }
    ...
    if (utility.isInitialized(this._parentRoutes))
    {
      ...
      {
        for (const methodIdentifier in CrudController.METHOD_IDENTIFIER)
        {
  ...
  async read (request, response, next = null, hooks = {}, allowedProperties = this._allowedProperties[CrudController.METHOD_IDENTIFIER.READ])
  {
    try
    {
      ...
      const parentRouteValues = utility.isInitialized(this._parentRoutes) ? this._extractParentRouteParameters(pathParameters) : null;
      ...
      {
        throw new BadRequestError("Query must be valid JSON.");
      }
      ...
      await SessionManager.exec(async () =>
                                {
                                  ...
                                  documents = await this._controllerService.read(query, session, this._parentRoutes, parentRouteValues);
      ...
      await SessionManager.exec(async () =>
                                {
                                  ...
                                  document = await this._controllerService.readOneById(_id, session, this._parentRoutes, parentRouteValues);
      ...
      await SessionManager.exec(async () =>
                                {
                                  ...
                                  document = await this._controllerService.createOne(body, session, this._parentRoutes, parentRouteValues);
                                  utility.isExist(hooks.after) ? await hooks.after(document, session) : null;
                                });
      ...
      await SessionManager.exec(async () =>
                                {
                                  ...
                                  document = await this._controllerService.updateOneByIdAndVersion(_id, version, body, session, this._parentRoutes, parentRouteValues);
                                  utility.isExist(hooks.after) ? await hooks.after(document, session) : null;
                                });
      ...
      await SessionManager.exec(async () =>
                                {
                                  ...
                                  utility.isExist(hooks.before) ? await hooks.before(_id, version, body, session) : null;
                                  document = await this._controllerService.replaceOneByIdAndVersion(_id, version, body, session, this._parentRoutes, parentRouteValues);
                                  utility.isExist(hooks.after) ? await hooks.after(document, session) : null;
                                });
      ...
      await SessionManager.exec(async () =>
                                {
                                  ...
                                  document = await this._controllerService.softDeleteOneByIdAndVersion(_id, version, session, this._parentRoutes, parentRouteValues);
                                  utility.isExist(hooks.after) ? await hooks.after(document, session) : null;
                                });
  ...
  async deleteOneByIdAndVersion (request, response, next = null, hooks = {}, allowedProperties = this._allowedProperties[CrudController.METHOD_IDENTIFIER.DELETE_ONE_BY_ID_AND_VERSION])
  {
    try
    {
      ...
      await SessionManager.exec(async () =>
                                {
                                  utility.isExist(hooks.before) ? await hooks.before(_id, version, session) : null;
                                  document = await this._controllerService.deleteOneByIdAndVersion(_id, version, session, this._parentRoutes, parentRouteValues);
                                  utility.isExist(hooks.after) ? await hooks.after(document, session) : null;
                                });
  ...
  _extractParentRouteParameters (pathParameters)
  {
