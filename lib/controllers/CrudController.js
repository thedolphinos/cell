...
class CrudController extends Controller
{
  ...
  async createOne (request, response, next = null, hooks = {})
  {
    try
    {
      ...
      utility.isExist(hooks.options) ? options = await hooks.options() : null;

      if (utility.isExist(hooks.body))
      {
        const result = await hooks.body(body, options);
        utility.isExist(result) && utility.isExist(result.body) ? body = result.body : null;
        utility.isExist(result) && utility.isExist(result.options) ? body = result.options : null;
      }

      const document = await this._controllerService.createOne(body, options);

      utility.isExist(hooks.options_finally) ? await hooks.options_finally(options) : null;
   ...
   * @return {Promise<void>}
   */
  async partialUpdateOneByIdAndVersion (request, response, next = null, hooks = {})
  {
    try
    {
      CrudController.validateControllerParameters(request, response, next);
      ...
      const document = await this._controllerService.partialUpdateOneByIdAndVersion(id, version, body, options);
