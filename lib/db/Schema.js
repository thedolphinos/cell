...
class Schema
{
   ...
   */
  constructor (dbName, collectionName, definition = undefined)
  {
      ...
      switch (error.code)
      {
        case 48:
          ...
          catch (error)
          {
            ...
            {
              case 9:
                throw new DeveloperError({en: "Invalid schema definition"});
              default:
                Logger.error(`MongoDB level error is occurred: (CODE ${error.code}) ${error.getMessage()}`);
            }
          }
  ...
  _validateAndCleanDefinitionForControllerOperation (definition)
  {
    ...
    if (utility.isExist(definition.controller))
    {
      ...
      for (const controllerOperation in definition.controller)
      {
        ...
        {
          throw new DeveloperError({en: `In '${this._dbName}.${this._collectionName}', you defined an invalid controller operation '${controllerOperation}'.`});
        }

        if (!_.isBoolean(definition.controller[controllerOperation]))
        {
          throw new DeveloperError({en: `In '${this._dbName}.${this._collectionName}', for controller operation '${controllerOperation}', you set an invalid value ${definition.controller[controllerOperation]}. It must be 'boolean'.`});
        }
      }
  ...
  _validateAndCleanDefinitionForMultilingualIndicator (definition)
  {
    ...
    if (utility.isExist(definition.isMultilingual))
    {
      ...
      {
        throw new DeveloperError({en: `In '${this._dbName}.${this._collectionName}', for multilingual indicator, you set an invalid value ${definition.isMultilingual}. It must be 'boolean'.`});
      }
  ...
  _makeDefinitionMultilingual (definition)
  {
    ...
    {
      throw new DeveloperError({en: "In order to make schema multilingual, you must place your languages to language safe."});
    }
