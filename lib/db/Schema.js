...
class Schema
{
   ...
   * @param {boolean} [options.isEnforceDefinition] - Indicates if `definition` will be enforced or not. Default is `true`. If provided, `definition` also must be provided.
   * @param {boolean} [options.isVersionBased] - Indicates if the schema is version based or not. Default is `false`.
   */
  constructor (dbName, collectionName, definition = undefined, options = undefined)
  {
    options = utility.init(options, {});
    options.isEnforceDefinition = utility.init(options.isEnforceDefinition, true);
    options.isVersionBased = utility.init(options.isVersionBased, false);
    this._validateConstructorParams(dbName, collectionName, definition, options);

    this._dbName = dbName;
    this._collectionName = collectionName;
    this._definition = definition;
    this._isVersionBased = options.isVersionBased;
    ...
    {
      if (options.isVersionBased)
      {
        this._definition.required = [
          "_root",
          "isRecent",
          ...this._definition.required
        ];
        ...
        this._rootSchema = new Schema(
          ...
          `root${collectionName.charAt(0).toUpperCase()}${collectionName.substring(1)}`,
          {
            ...
            required: ["_id", "version", "isSoftDeleted"],
      ...
      if (options.isEnforceDefinition) // definition enforcement must be called after then version adaptations, because version adaptations changes the definition and those changes must be also reflected.
      {
   ...
   * @param {boolean} [options.isEnforceDefinition]
   * @param {boolean} [options.isVersionBased]
   * @protected
   */
  _validateConstructorParams (dbName, collectionName, definition = undefined, options = undefined)
  {
    if (!_.isString(dbName) ||
        ...
        (utility.isExist(options.isEnforceDefinition) && !_.isBoolean(options.isEnforceDefinition)) ||
        (utility.isExist(options.isVersionBased) && !_.isBoolean(options.isVersionBased)))
    {
      throw new InvalidArgumentsError(ErrorSafe.get().DEV_1);
    }

    if (options?.isEnforceDefinition && !utility.isExist(definition))
    {
      throw new InvalidArgumentsError(
        {
          ...ErrorSafe.get().DEV_0,
          message: {
            en: `${dbName}.${collectionName}'s schema definition enforced in the options, but the schema defined.`,
            tr: `${dbName}.${collectionName}'nin şema tanımı seçeneklerde zorunlu tutulmuş, fakat şema tanımlanmamış.`
          }
        }
      );
    }
