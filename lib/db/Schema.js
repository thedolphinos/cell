...
class Schema
{
  ...
  constructor (dbName, collectionName, definition = undefined, options = undefined)
  {
    ...
    if (options.isValidationEnabled) // definition enforcement must be called after then version adaptations, because version adaptations changes the definition and those changes must be also reflected.
    {
      const collMod = this._collectionName;
      const validator = options.isValidationEnabled ? {$jsonSchema: this._generateJsonSchema()} : {};
      const validationLevel = options.isValidationEnabled ? "moderate" : "off";

      try
      {
        this._db.createCollection(collMod, {validator, validationLevel}); // since the method is called inside the constructor and constructors cannot be asynchronous, if update the definition, and if you do an operation on the collection that the schema is related right after the instantiation of the schema the reflection of the updates might be later than the operation which might seem you as a bug.
      }
      catch (error)
      {
        switch (error.code)
        {
          case 48:
            try
            {
              this._db.command({collMod, validator, validationLevel});
            }
            catch (error)
            {
              switch (error.code)
