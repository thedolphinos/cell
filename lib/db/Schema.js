...
class Schema
{
  ...
  static Definition = {
    ...
    CommonAuthProperties: {
      auth: {
        ...
        required: ["isBlocked", "numberOfFailedLoginAttempts", "numberOfFailedChangePasswordAttempts"],
        properties: {
          isBlocked: {
            bsonType: Schema.DataType.Boolean
          },
      ...
      switch (error.code)
      {
        case 48:
          ...
          catch (error)
          {
            ...
            {
              case 9:
                throw new DeveloperError(ERROR_DATA.DB_INVALID_SCHEMA);
              default:
  ...
  _validateAndCleanDefinitionForControllerOperation (definition)
  {
    ...
    if (utility.isExist(definition.controller))
    {
      ...
      for (const controllerOperation in definition.controller)
      {
        ...
        {
          throw new DeveloperError(ERROR_DATA.SCHEMA_CUSTOM_INVALID_CONTROLLER_OP, this._dbName, this._collectionName, controllerOperation);
        }

        if (!_.isBoolean(definition.controller[controllerOperation]))
        {
          throw new DeveloperError(ERROR_DATA.SCHEMA_CUSTOM_INVALID_CONTROLLER_OP_VALUE, this._dbName, this._collectionName, controllerOperation, definition.controller[controllerOperation]);
        }
      }
  ...
  _validateAndCleanDefinitionForMultilingualIndicator (definition)
  {
    ...
    if (utility.isExist(definition.isMultilingual))
    {
      ...
      {
        throw new DeveloperError(ERROR_DATA.SCHEMA_CUSTOM_INVALID_CONTROLLER_OP_VALUE);
      }
  ...
  _makeDefinitionMultilingual (definition)
  {
    ...
    {
      throw new DeveloperError(ERROR_DATA.LANGUAGE_SAFE);
    }
