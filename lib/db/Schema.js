...
class Schema
{
  ...
  constructor (dbName, collectionName, definition = undefined, options = undefined)
  {
    ...
    if (utility.isExist(definition) && options.isEnforceDefinition)
    {
   ...
   *
   * @type {{createOne: string, replaceOneByIdAndVersion: string, read: string, updateOneByIdAndVersion: string}}
   */
  ...
  _validateAndCleanDefinitionForForbiddenPropertyIndicator (definition)
  {
    ...
    {
      if (!_.isArray(definition.forbiddenForPersonas))
      {
        throw new DeveloperError(ErrorSafe.get().DB_INVALID_SCHEMA);
      }
