    ...
    if (utility.isInitialized(definition))
    {
      ...
      switch (Schema.identifyBsonType(definition))
      {
        case Schema.DataType.Object:
          /*
           converts,
              biography: {
                ...
                properties: {
                  title: {
                    bsonType: Schema.DataType.String
                  },
                  description: {
                    bsonType: Schema.DataType.String
                  }
           ...
           */
          const isMultilingual = definition.isMultilingual;
          ...
          if (isMultilingual)
          {
            const definitionOfEachLanguage = {}; // the same definition for each language must be generated. all of the property references must be deleted from the definition.

            for (const key in definition)
            {
              definitionOfEachLanguage[key] = _.cloneDeep(definition[key]);
              delete definition[key];
            }

            // create new references of each language under the definition.
            ...
            {
              definition.properties[language] = definitionOfEachLanguage;
            }
          }
