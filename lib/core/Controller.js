...
class Controller
{
  ...
  static _extractAndAuthorizeRequestElement (requestElement, request, allowedProperties = undefined, propertyDefinition = undefined, isRequired = undefined)
  {
    ...
    switch (requestElement)
    {
      ...
      case Controller._REQUEST_ELEMENT.BODY:
      {
        ...
        if (isInitialized)
        {
          Controller._authorizePropertiesForPropertyDefinition(extractedRequestElement, propertyDefinition, true);
        }
   ...
   * @typedef {{[]: string | PropertyDefinition} | Array<{[]: string | PropertyDefinition}>} PropertyDefinition
   * @param {PropertyDefinition} propertyDefinition - Represents property definition. Contains information about data type and required status.
   * @param {boolean} isRecursiveCall - Controls validation of `propertyDefinition` which must not be made on the recursive calls.
   * @protected
   */
  static _authorizePropertiesForPropertyDefinition (object, propertyDefinition, isRecursiveCall = undefined)
  {
    isRecursiveCall = utility.init(isRecursiveCall, false);
    ...
    if (isString)
    {
      ...
      switch (propertyDefinition)
      {
        ...
        case DataType.Date:
        {
          isInvalidType = !utility.isValidDate(object);
          break;
        }
        case DataType.Any:
        {
    ...
    else if (isPlainObject)
    {
      ...
      {
        const isRequired = !key.startsWith("."); // optional property names starts with `.`.
        const propertyName = !isRequired ? key.slice(1) : key;
        const propertyValue = clonedObject[propertyName];
        delete clonedObject[propertyName];

        if (utility.isExist(propertyValue))
        {
          Controller._authorizePropertiesForPropertyDefinition(propertyValue, propertyDefinition[key], true);
        }
        else
        {
          if (isRequired)
    ...
    else if (isArray)
    {
      ...
      for (const item of clonedObject)
      {
        Controller._authorizePropertiesForPropertyDefinition(item, propertyDefinition[0], true);
      }
    }
  }
  ...
  async _sendResponse (request, response, statusCode = undefined, data = undefined)
  {
    ...
    if (utility.isExist(response?.locals?.newToken))
    {
      responseData.token = response.locals.newToken;
    }
