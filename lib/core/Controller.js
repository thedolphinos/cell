...
class Controller
{
  ...
  async verifyPublic (request, response, next, hooks = undefined, allowedProperties = undefined)
  {
    try
    {
      ...
      utility.isExist(hooks.locals) ? await hooks.locals(request, headers) : undefined;
  ...
  _sendResponseWhenError (response, error)
  {
    try
    {
      ...
      if (!(error instanceof InternalServerError))
      {
        ...
        {
          Logger.error(`MongoDB level error is occurred: (CODE ${error.code}) ${error.getMessage()}`, error);
