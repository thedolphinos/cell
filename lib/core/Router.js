...
class Router
{
   ...
   * @typedef {{[isEnabled]: boolean, [allowedPropertiesForRequestElements]: AllowedPropertiesForRequestElements, searchFields: Array<string>}} RouteDefinitionForSearch
   * @typedef {{[isEnabled]: boolean, [allowedPropertiesForRequestElements]: AllowedPropertiesForRequestElements}} RouteDefinition
   * @param {{[SEARCH]: RouteDefinitionForSearch, [READ]: RouteDefinition, [READ_ONE_BY_ID]: RouteDefinition, [CREATE_ONE]: RouteDefinition, [UPDATE_ONE_BY_ID_AND_VERSION]: RouteDefinition, [REPLACE_ONE_BY_ID_AND_VERSION]: RouteDefinition, [SOFT_DELETE_ONE_BY_ID_AND_VERSION]: RouteDefinition, [DELETE_ONE_BY_ID_AND_VERSION]: RouteDefinition}} routesDefinitions - Represents route definitions where each contains enabled status and allowed properties for request elements.
  ...
  static generateRoutes (router, routesDefinitions, crudController, verify = undefined, extraVerificationFunctions = undefined)
  {
    ...
    if (apiType === Controller.API_TYPE.REST)
    {
      ...
      if (utility.isExist(routesDefinitions.SEARCH) &&
          utility.isExist(routesDefinitions.READ) &&
          routesDefinitions.SEARCH.isEnabled && routesDefinitions.READ.isEnabled)
      {
        throw new InvalidArgumentsError(ErrorSafe.get().DEV_1);
      }
    ...
    for (const routeName in routesDefinitions)
    {
      ...
      if (routeDefinition.isEnabled)
      {
        ...
        router.route(routePath)[httpMethodName]((request, response, next) => crudController[crudControllerMethodName](request, response, next, null, routeDefinition.allowedPropertiesForRequestElements, crudControllerMethodName === Router._CRUD_CONTROLLER_METHOD_NAME.SEARCH ? routeDefinition.searchFields : null));
      }
    }
  }
}
