  ...
  _validateAndConvertObjectCandidate (candidate, schemaDefinition, convertedCandidate = null)
  {
    ...
    if (schemaDefinition.isMultilingual)
    {
      for (const language in candidate)
      {
        ...
        if (!utility.isExist(convertedCandidate[language]))
        {
        ...
        for (const key in subCandidate)
        {
          ...
          const value = subCandidate[key];

          switch (Schema.identifyBsonType(schemaDefinition.properties[key]))
          {
            case Schema.DataType.Boolean:
              convertedCandidate[language][key] = this._validateAndConvertBooleanCandidate(value);
              break;
            case Schema.DataType.Int:
              convertedCandidate[language][key] = this._validateAndConvertIntCandidate(value);
              break;
            case Schema.DataType.Double:
              convertedCandidate[language][key] = this._validateAndConvertDoubleCandidate(value);
              break;
            case Schema.DataType.String:
              convertedCandidate[language][key] = this._validateAndConvertStringCandidate(value);
              break;
            case Schema.DataType.ObjectId:
              convertedCandidate[language][key] = this._validateAndConvertObjectIdCandidate(value);
              break;
            case Schema.DataType.Date:
              convertedCandidate[language][key] = this._validateAndConvertDateCandidate(value);
              break;
            case Schema.DataType.Object:
              convertedCandidate[language][key] = this._validateAndConvertDocumentCandidate(value, schemaDefinition.properties[key], convertedCandidate[language][key]);
              break;
            case Schema.DataType.Array:
              convertedCandidate[language][key] = this._validateAndConvertArrayCandidate(value, schemaDefinition.properties[key], convertedCandidate[language][key]);
              break;
            default:
              convertedCandidate[language][key] = value; // if BSON type is not specified, leave value as is.
          }
        }
      }
    }
