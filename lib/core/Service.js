...
{
  /**
   * Creates a service instance by injecting dependencies, if provided.
   *
   * @param {Object} [options]
   * @param {Object} [options.dependencies] - Key value pair of names and injected services using the injector.
   * @param {Array} [options.fromConstructorParameters] - Must be presented if dependencies are presented.
   * @param {Service} [options.from] - Placed by the framework.
   */
  constructor (options = undefined)
  {
    if (utility.isExist(options) &&
        (
          (utility.isExist(options.dependencies) && (!_.isPlainObject(options.dependencies) || !utility.isExist(options.fromConstructorParameters))) ||
          (utility.isExist(options.fromConstructorParameters) && !_.isArray(options.fromConstructorParameters)) ||
          (utility.isExist(options.from) && !(options.from instanceof Service))
        )
    )
    {
    ...
    if (utility.isExist(options) && utility.isExist(options.dependencies))
    {
      for (const dependencyName in options.dependencies)
      {
        const {injector, className, constructorParameters} = options.dependencies[dependencyName];

        if (utility.isExist(options.from) && options.from.constructor.name === className)
        {
          const injectedInstance = injector.getInstance(className, options.fromConstructorParameters);

          if (utility.isExist(injectedInstance))
          {
            this[dependencyName] = injectedInstance;
            continue;
          }

          if (_.isEqual(constructorParameters, options.fromConstructorParameters))
          {
            this[dependencyName] = options.from;
            continue;
          }

          this[dependencyName] = injector.instantiate(options.from.constructor.name, options.fromConstructorParameters, this, [...arguments]);
          continue;
        }

        const injectedInstance = injector.getInstance(className, constructorParameters);
        ...
        this[dependencyName] = injector.instantiate(className, constructorParameters, this, options.fromConstructorParameters);
      }
    }
  }
  ...
  static validateAndConvertObjectCandidate (candidate, schemaDefinition, convertedCandidate = undefined, layer = undefined)
  {
    ...
    else
    {
      for (const key in candidate)
      {
        ...
        {
          // schema definiton may not have properties which is fine in some layers.
        }
        ...
        if (!utility.isExist(subCandidateDefinition))
        {
          ...
          {
            case Service.LAYER.CONTROLLER:
              throw new BadRequestError(); // the client sent a property which is not in the schema definition.
            case Service.LAYER.APPLICATION:
              continue;
            case Service.LAYER.DB:
              continue;
