...
class Service
{
   ...
   * @param isSessionEnabled - The session enabler in the hooks.
   * @return {Promise<void>}
   * @protected
   */
  _exec = async (func, session, isSessionEnabled) =>
  {
    ...
    else
    {
      ...
      if (isSessionEnabled)
      {
        ...
        try
        {
          await session.withTransaction(func, this.dbOperation.transactionOptions);
        }
        catch (error)
        {
          throw error;
