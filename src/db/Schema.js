...
class Schema
{
    ...
    constructor (dbName, collectionName, definition = undefined, options = undefined)
    {
        ...
        if (this._isAddCommonProperties)
        {
            ...
            if (!utility.isExist(this._definition.properties.createdAt))
            {
                this._definition.properties.createdAt = {
                    bsonType: BsonType.Date,
                    forbiddenForPersonas: [Schema.SPECIAL_FORBIDDEN_FOR_PERSONA.ALL]
                };
                this._definition.required.push("createdAt");
            }

            if (!utility.isExist(this._definition.properties.updatedAt))
            {
                this._definition.properties.updatedAt = {
                    bsonType: BsonType.Date,
                    forbiddenForPersonas: [Schema.SPECIAL_FORBIDDEN_FOR_PERSONA.ALL]
                };
            }

            if (!utility.isExist(this._definition.properties.softDeletedAt))
            {
                this._definition.properties.softDeletedAt = {
                    bsonType: BsonType.Date,
                    forbiddenForPersonas: [Schema.SPECIAL_FORBIDDEN_FOR_PERSONA.ALL]
                };
            }
